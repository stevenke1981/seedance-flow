...
20. 節點畫布提供可切換的懸停預覽；開啟時滑鼠停留或鍵盤聚焦節點會顯示包含節點欄位完整內容的固定浮窗，關閉時不得顯示浮窗，偏好只保存於瀏覽器本機。
21. 懸停預覽浮窗內的文字、多行文字與下拉欄位可直接編輯；修改必須同步目前節點、Inspector、提示詞與 localStorage，滑鼠由節點移入浮窗時不得中斷編輯。
22. 節點庫、畫布與懸停浮窗必須為八種節點顯示一致且可辨識的圖示；圖示資產由內建生圖工具產出並隨 release 產物發布。
...
- 節點畫布的懸停預覽可由工具列開關控制；桌面滑鼠移入與鍵盤 Tab 聚焦均可顯示／關閉完整節點內容，拖曳、選取與欄位編輯不被浮窗攔截。
- 節點畫布的懸停預覽可由工具列開關控制；浮窗內可編輯文字、textarea 與 select，修改會同步 Inspector、提示詞與本機工作流，且滑鼠移入浮窗時保持開啟。
- 節點庫、畫布節點與懸停浮窗均顯示對應 Scene／Character／Camera／Motion／Style／Audio／Output／Reference 圖示，390px 版面不產生水平溢位。
